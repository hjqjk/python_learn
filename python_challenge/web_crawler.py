...
# 第一个参数。 例子：http://music.163.com/#/song?id=186016
# rid就是'R_SO_4_'+'歌曲的id'，offset的取值为:(评论页数-1)*20,total第一页为true，其余页为false
...
# 获取参数
def get_params(page): # page 为传入页数
    iv = "0102030405060708"  # 密钥偏移量
    first_key = forth_param
    second_key = 16 * 'F'
    if(page == 1): # 如果为第一页
        first_param = '{rid:"", offset:"0", total:"true", limit:"20", csrf_token:""}'
        h_encText = AES_encrypt(first_param, first_key, iv)  # 第一层解密
    else:   # 非第一页
        offset = str((page-1)*20)
        first_param = '{rid:"", offset:"%s", total:"%s", limit:"20", csrf_token:""}' % (offset, 'false')
        h_encText = AES_encrypt(first_param, first_key, iv) # 第一层解密
    h_encText = AES_encrypt(h_encText, second_key, iv)  # 第二层解密
    return h_encText
...
def get_json(url, params, encSecKey):
    ...
    # 模拟浏览器访问，并获得返回结果。data为url的参数
    response = requests.post(url, headers=headers, data=data, proxies=proxies)
    return response.content

# 抓取热门评论，返回热评列表
def get_hot_comments(url):
    hot_comments_list = []   # 热评列表
    hot_comments_list.append(u"用户ID 用户昵称 用户头像地址 评论时间 点赞总数 评论内容\n")
    params = get_params(1)   # 第一页
    encSecKey = get_encSecKey()
    json_text = get_json(url,params,encSecKey) # 获取第一页评论内容
    json_dict = json.loads(json_text)
#   pprint(json_dict) # 测试，用json可视化格式查看热点评论
    ...
    for item in hot_comments:
        ...
        avatarUrl = item['user']['avatarUrl'] # 头像地址o
        comment_info = str(userId) + " " + nickname + " " + avatarUrl + " " + str(comment_time) + " " + str(likedCount) + " " + comment + u"\n"
...
if __name__ == "__main__":
    ...
    save_to_file(hot_comments_list,filename)
