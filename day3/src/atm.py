...
def show_menu(): #显示登录页面的菜单
    ...
    while True:
        ...
        if operation in [1,2,3,4,5]: #根据选择，返回选择号
            return operation
            break
        else:
            print "输入错误，请重新输入。"
    ...
    while account.has_key(card_number):  #用户存在
        if account[card_number]['locked'] == 1:  #用户被锁
            ...
            if error_count == 2:  # 如果密码输错三次，则锁住该用户
                ...
                account[card_number]['locked'] = 1   #密码输错三次，则将该卡号标记为锁住状态
                return 0 #不成功状态
                break
            else:
                print '你的密码输错了，请再次输入!'
...
def fast_cash(account,card_number): #提现功能
    while True:
        cash = input("输入你要取的金额:") 
        if cash*1.05 > account[card_number]['balance']: #所取金额及利息 > 余额 ，则余额不足
            print '余额不足，不能取'
            break
            ...
            flush(account,'../db/account.db') #实时刷新到文件中，否则会发生金额不一致的情况
...
def query(account,card_number): #查询余额和交易明细
    print "你的账号余额是：%s 元" % account[card_number]['balance']  #查询余额
    f = open('../db/user_deal.log','r') #打开交易记录文件
    for line in f.xreadlines():
        loglist = line.split()
        if loglist[0] == card_number: #筛选指定卡号的交易记录
...
def transfer_accounts(account,card_number): #转账
    ...
    if target in account: #判断收款账户是否存在
        if cash <= account[card_number]['balance']:  #余额充足才能转账
