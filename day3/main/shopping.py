...
def shopping(goods): #购物
    ...
    while True:
        ...
        if (buyNo-1) in range(len(goods)):
            total_money += goods[buyNo-1]['price']
            shopping_cart.append(goods[buyNo-1]['name'])
...
def goods_list(goods):#打印商品列表
    ...
    for goods_list in goods: #循环读取商品信息
        print "%d.\t商品名:%s\t单价(元):%d\t库存量(个):%d" % (i+1,goods_list['name'],goods_list['price'],goods_list['inventory'])
        i += 1
